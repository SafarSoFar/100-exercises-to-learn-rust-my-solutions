...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket{
        Ok(tick) => tick,
        Err(error_message) =>{
            if error_message.contains("Title"){
                panic!("{}", error_message);
            }
            else if error_message.contains("Description"){
                Ticket{title:title, description: String::from("Description not provided"), status: status, }
            }
            else{
                panic!(" ");
            }
