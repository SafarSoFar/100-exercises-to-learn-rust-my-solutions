...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket{
        ...
        Err(error_type) => match error_type{
            ...
            TicketNewError::InvalidDescription {message: _} => Ticket{title, description : String::from("Description not provided"), status}
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::InvalidTitle{message :"Title cannot be empty".to_string()});
        }
        if title.len() > 50 {
            return Err(TicketNewError::InvalidTitle{message :"Title cannot be longer than 50 bytes".to_string()});
        }
        if description.is_empty() {
            return Err(TicketNewError::InvalidDescription{message : "Description cannot be empty".to_string()});
        }
        if description.len() > 500 {
            return Err(TicketNewError::InvalidDescription{message :"Description cannot be longer than 500 bytes".to_string()});
        }
